...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-display text-xl font-700 tracking-tight" style={{ color: 'var(--ink)', fontWeight: 700 }}>
              Wanderlust
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-8">
            <Link href="/" className="section-label transition-colors hover:text-amber-700"
                ...
                {user ? (
                  <>
                    ...
                    <div className="relative">
                      ...
                      {dropdownOpen && (
                        <div className="absolute right-0 mt-1.5 w-52 rounded overflow-hidden animate-fade-in"
                          style={{ background: 'var(--white)', border: '1px solid var(--border)', boxShadow: '0 12px 40px rgba(13,13,13,0.12)' }}>
          ...
          {/* Mobile hamburger */}
          <button className="md:hidden p-2 rounded transition-colors hover:bg-sand"
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden border-t animate-fade-in" style={{ borderColor: 'var(--border)', background: 'var(--white)' }}>
          <div className="max-w-7xl mx-auto px-6 py-4 space-y-1">
            {user ? (
              <>
                <div className="flex items-center gap-3 py-3 mb-2 border-b" style={{ borderColor: 'var(--border)' }}>
                  <Image src={getAvatarUrl(user)} alt={user.name} width={36} height={36}
                ...
                <Link href="/create" className="block px-3 py-2.5 text-sm font-medium rounded"
                  style={{ color: 'var(--gold)' }}>+ New Experience</Link>
                <Link href="/profile" className="block px-3 py-2.5 text-sm rounded"
                  style={{ color: 'var(--ink-soft)' }}>My Profile</Link>
                <button onClick={handleLogout} className="block w-full text-left px-3 py-2.5 text-sm rounded"
                  style={{ color: 'var(--coral)' }}>Sign Out</button>
              </>
            ) : (
              <>
                <Link href="/auth/login" className="block px-3 py-2.5 text-sm" style={{ color: 'var(--ink-soft)' }}>Sign In</Link>
                <Link href="/auth/register" className="block px-3 py-2.5 text-sm font-medium" style={{ color: 'var(--gold)' }}>Get Started</Link>
              </>
            )}
          </div>
        </div>
